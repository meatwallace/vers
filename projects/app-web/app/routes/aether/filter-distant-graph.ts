...
import type { AetherEdgeMap, AetherGraph, AetherNodeMap } from './types.ts';
...
export function filterDistanceGraph(
  ...
  for (const [id, node] of Object.entries(graphData.nodes)) {
    const nodePosition = getScenePosition(node.position);
  ...
  for (const [id, edge] of Object.entries(graphData.edges)) {
    const start = getScenePosition(edge.start);
    const end = getScenePosition(edge.end);
